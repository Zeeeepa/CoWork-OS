...
vi.mock('botbuilder', () => {
  ...
  const MockCloudAdapter = class {
    ...
    continueConversationAsync = vi.fn().mockImplementation(async (appId: string, ref: any, callback: any) => {
      const mockContext = {
...
describe('TeamsAdapter', () => {
  ...
  describe('editMessage', () => {
    it('should throw error when not connected', async () => {
      await expect(adapter.editMessage('conv-123', 'msg-123', 'Updated'))
        .rejects.toThrow('Teams bot is not connected');
    });
    ...
    it('should throw error when no conversation reference exists', async () => {
      ...
      await expect(adapter.editMessage('unknown-chat', 'msg-123', 'Updated'))
        .rejects.toThrow('No conversation reference found');
  ...
  describe('deleteMessage', () => {
    it('should throw error when not connected', async () => {
      await expect(adapter.deleteMessage('conv-123', 'msg-123'))
        .rejects.toThrow('Teams bot is not connected');
    });
    ...
    it('should throw error when no conversation reference exists', async () => {
      ...
      await expect(adapter.deleteMessage('unknown-chat', 'msg-123'))
        .rejects.toThrow('No conversation reference found');
  ...
  describe('sendDocument', () => {
    it('should throw error when not connected', async () => {
      await expect(adapter.sendDocument('conv-123', '/path/to/file.pdf'))
        .rejects.toThrow('Teams bot is not connected');
    });
    ...
    it('should throw error when file does not exist', async () => {
      ...
      await expect(adapter.sendDocument('conv-123', '/nonexistent/file.pdf'))
