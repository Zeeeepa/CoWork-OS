...
describe('SearchTools', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(SearchProviderFactory.loadSettings).mockReturnValue({ primaryProvider: 'tavily' } as any);
  });
  ...
  describe('webSearch', () => {
    ...
    it('should preserve provider in error response', async () => {
      vi.mocked(SearchProviderFactory.loadSettings).mockReturnValue({ primaryProvider: 'serpapi' } as any);
      vi.mocked(SearchProviderFactory.searchWithFallback).mockRejectedValue(
        new Error('API error')
      );
