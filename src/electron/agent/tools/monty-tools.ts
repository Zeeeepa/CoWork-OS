...
export class MontyTools {
    ...
    return [
      {
        ...
        input_schema: {
          ...
          properties: {
            code: {
              type: 'string',
              description: 'Monty (Python-subset) code. The value of the last expression is returned.',
            },
            inputs: {
              type: 'object',
              description: 'Arbitrary JSON-like input object available as `input` inside the script.',
            },
            limits: {
              type: 'object',
      ...
      },
      {
        ...
        input_schema: {
          type: 'object',
          properties: {
            transform: { type: 'string', description: 'Transform name (file name without extension).' },
            inputPath: { type: 'string', description: 'Input file path (workspace-relative preferred).' },
            outputPath: { type: 'string', description: 'Output file path (defaults to "<inputPath>.monty.out").' },
            overwrite: { type: 'boolean', description: 'Overwrite output file if it exists (default: false).' },
            maxInputBytes: { type: 'number', description: 'Maximum bytes to read from input file (default: 1_000_000).' },
            limits: {
              type: 'object',
              description: 'Optional resource limits (clamped).',
              properties: {
      ...
      },
      {
        ...
        input_schema: {
          ...
          properties: {
            ...
            mode: { type: 'string', enum: ['first', 'all'], description: 'Return only the first JSON value or all found.' },
            allowRepair: { type: 'boolean', description: 'Attempt light JSON repairs (trailing commas, unquoted keys, etc.). Default: true.' },
            maxResults: { type: 'number', description: 'Max number of JSON values to return when mode="all" (default: 5).' },
          },
          required: ['text'],
        },
      },
  ...
  private async loadTransformCode(name: string): Promise<{ id: string; absPath: string; code: string }> {
  ...
  async transformFile(input: MontyTransformFileInput): Promise<any> {
    ...
    const outputPath = (input.outputPath && String(input.outputPath).trim().length > 0)
    ...
    if (!overwrite) {
      try {
        ...
        return { success: false, error: `Output path exists: ${outputPath} (set overwrite=true to overwrite)` };
      } catch {
        // does not exist
      }
    }
