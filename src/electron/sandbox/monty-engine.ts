...
export type MontyRunResult =
  | { ok: true; output: unknown }
  | { ok: false; error: MontyRunError };
...
async function loadMontyModule(): Promise<MontyModule> {
  ...
  const dynamicImport = new Function('specifier', 'return import(specifier)') as (specifier: string) => Promise<MontyModule>;
...
export function createMontySafeStdlib(): MontyStdlib {
  return {
    ...
    json_stringify: (value: unknown, options?: unknown) => {
      ...
      const maxOutputChars = typeof opts?.max_output_chars === 'number' ? opts.max_output_chars : opts?.maxOutputChars;
      return stableJsonStringify(montyValueToJs(value), { indent, sortKeys, maxOutputChars });
    },
    json_extract: (text: unknown) => {
      const s = typeof text === 'string' ? text : String(text ?? '');
      return extractJsonValues(s, { maxResults: 20, allowRepair: true, maxCandidateChars: 200_000 });
    },
...
export function clampMontyLimits(requested: MontyResourceLimits | undefined, maxima: MontyResourceLimits): MontyResourceLimits | undefined {
  if (!requested) return undefined;
  const out: MontyResourceLimits = {};

  const maxAllocations = clampNumber(requested.maxAllocations, 1, maxima.maxAllocations ?? 2_000_000);
  if (maxAllocations !== undefined) out.maxAllocations = maxAllocations;

  const maxDurationSecs = clampNumber(requested.maxDurationSecs, 0.01, maxima.maxDurationSecs ?? 30);
  if (maxDurationSecs !== undefined) out.maxDurationSecs = maxDurationSecs;

  const maxMemory = clampNumber(requested.maxMemory, 1024 * 1024, maxima.maxMemory ?? 512 * 1024 * 1024);
  ...
  const maxRecursionDepth = clampNumber(requested.maxRecursionDepth, 10, maxima.maxRecursionDepth ?? 5000);
...
export async function runMontyCode(options: RunMontyOptions): Promise<MontyRunResult> {
  ...
  try {
    ...
    return { ok: false, error: { kind: 'load', message: `Failed to load @pydantic/monty: ${err?.message || String(err)}` } };
  }
  ...
  try {
    ...
    try {
      ...
      if (err instanceof MontySyntaxError) {
        return { ok: false, error: { kind: 'syntax', message: err.message, display: err.display('type-msg') } };
      }
      if (err instanceof MontyTypingError) {
        return { ok: false, error: { kind: 'typing', message: err.message, display: err.displayDiagnostics('concise', false) } };
      }
